...
public class HttpIntHawbDelete {
    public static SoapObject deleteIntHawb(String userBumen, String userName, String userPass, String loginFlag, String mawb, String hno){
        // 定义SoapHeader，加入4个节点
        ...
        SoapObject rpc = new SoapObject(HttpCommons.NAME_SPACE, HttpCommons.DELETE_INT_HAWB_METHOD_NAME);

        // 设置调用webservice接口需要传入的参数
        rpc.addProperty("Mawb", mawb);
        rpc.addProperty("Hno", hno);
        ...
        try {
            // 调用webservice
            transportSE.call(HttpCommons.DELETE_INT_HAWB_METHOD_ACTION, envelope);
        } catch (Exception e){
            e.printStackTrace();
        }
