...
public class HttpCGOSplitSubLineArrival {
    public static SoapObject cGOSplitSubLineArrival(String userBumen, String userName, String userPass, String loginFlag, String xml) {
        // 定义SoapHeader，加入4个节点
        ...
        SoapObject rpc = new SoapObject(HttpCommons.NAME_SPACE, HttpCommons.CGO_SPLIT_SUB_LINE_ARRIVAL_NAME);

        // 设置调用webservice接口需要传入的参数
        rpc.addProperty("rchXml", xml);
        ...
        try {
            // 调用webservice
            transportSE.call(HttpCommons.CGO_SPLIT_SUB_LINE_ARRIVAL_ACTION, envelope);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String getSpilteXml(String RearchID, String PC, String Weight, String Volume) {
        String xml = new String("<RearchInfo>"
                + "<RearchID>" + RearchID + "</RearchID>"
                +"<PC>" + PC + "</PC>"
