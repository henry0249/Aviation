...
public class HttpPrepareAWBDelete {
    // mawb主运单号
    public static SoapObject deleteAWBdetail(String userBumen, String userName, String userPass, String loginFlag, String mawb){
        // 定义SoapHeader，加入4个节点
        ...
        SoapObject rpc = new SoapObject(HttpCommons.NAME_SPACE, HttpCommons.AWB_DELETE_METHOD_NAME);

        // 设置调用webservice接口需要传入的参数
        rpc.addProperty("Mawb", mawb);
        ...
        try {
            // 调用webservice
            transportSE.call(HttpCommons.AWB_DELETE_METHOD_ACTION, envelope);
        } catch (Exception e){
            e.printStackTrace();
        }
