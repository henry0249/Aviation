...
//        rpc.addProperty("ErrString", "");
    ...
    //region 重载的函数
    public void requstAync(final Context context, final String methodName, final String methodAction, final Map<String, String> params, final CallBack callBack,final String rePage) {
        new AsyncTask<Void, Void, String>() {
            @Override
            protected String doInBackground(Void... voids) {
                if (isCancelled()) {
    ...
    private SoapObject callService(Context context, String methodName, String methodAction, Map<String, String> params,String rePage) {
        // 定义SoapHeader，加入4个节点
        Element[] healder = new Element[1];
        healder[0] = new Element().createElement(HttpCommons.NAME_SPACE, "AuthHeaderNKG");
        ...
        if (rePage == "one") {
            transportSE = new HttpTransportSE(HttpCommons.CGO_END_POINT);
        } else {
            transportSE = new HttpTransportSE(HttpCommons.END_POINT);
        }
