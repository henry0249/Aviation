package com.example.administrator.aviation.http.getIntdeclareinfo;
...
public class HttpIntDeclareInfo {
    public static SoapObject getIntDeclareInfoDetail(String userBumen, String userName, String userPass, String loginFlag, String xml){
        // 定义SoapHeader，加入4个节点
        ...
        SoapObject rpc = new SoapObject(HttpCommons.NAME_SPACE, HttpCommons.CGO_GET_EXPORT_DECLARE_INFO_OF_ALL_NAME);

        // 设置调用webservice接口需要传入的参数
        rpc.addProperty("dXml", xml);
        ...
        try {
            // 调用webservice
            transportSE.call(HttpCommons.CGO_GET_EXPORT_DECLARE_INFO_OF_ALL_ACTION, envelope);
        } catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static String getIntDeclareInfoXml(String Mawb, String begintime, String endTime) {
        String xml = new String("<ArrivalCondition>"
                +"<Mawb>"+Mawb+"</Mawb>"
