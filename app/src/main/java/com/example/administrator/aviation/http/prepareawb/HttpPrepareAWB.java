...
public class HttpPrepareAWB {
    public static SoapObject getAWBInfo(String userBumen, String userName, String userPass, String loginFlag){
        // 定义SoapHeader，加入4个节点
        ...
        SoapObject rpc = new SoapObject(HttpCommons.NAME_SPACE, HttpCommons.AWB_METHOD_NAME);

        // 设置调用webservice接口需要传入的参数
        rpc.addProperty("Mawb", "");
        ...
        try {
            // 调用webservice
            transportSE.call(HttpCommons.AWB_SOAP_ACTION, envelope);
        } catch (Exception e){
            e.printStackTrace();
        }
